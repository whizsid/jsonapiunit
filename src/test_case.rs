...
impl TestCase {
    ...
    pub fn request(&self)->Request {
        match self.json.get("request") {
            Some(request)=>{
                Request::new(request.as_object().unwrap().to_owned())
            }
            None =>{
                panic!("Not containing request")
            }
        }
    }
    ...
    pub fn response(&self)->Response {
        match self.json.get("response") {
            Some(response)=>{
                Response::new(response.as_object().unwrap().to_owned())
            }
            None =>{
                panic!("Not containing response")
            }
        }
    }
...
impl Request {
    ...
    pub fn body(&self)->Option<Map<String,Value>> {
        match self.json.get("body") {
            Some(body)=>{
                Some(body.as_object().unwrap().to_owned())
            }
            None=>{
                None
            }
