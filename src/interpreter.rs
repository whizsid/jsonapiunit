...
impl Interpreter {
                    ...
                    "}}\"" => {
                        ...
                        match format.get(3..4).unwrap() {
                            ">" => {
                                ...
                                let variable_type =
                                    format.get((colon_offset + 1)..(format.len() - 3)).unwrap();
                                ...
                                let mut entered_value = from_str(&variable_value.clone())
                                    .expect("Invalid value entered!");

                                let check_type = type_check(variable_type, &entered_value);

                                if !check_type {
                                    variable_value = get_default_value(variable_type).expect(
                                        &format!("Unsupported type given for {}.", variable_name),
                                    );
                            ...
                            _ => {
                                match self
                                    .variables
                                    .get(&format.get(3..(format.len() - 3)).unwrap())
                                {
                                    Some(variable) => format!("{}", variable.value),
                                    None => panic!("Can not find a variable named {}", format),
                                }
                            }
                        }
                    }
    ...
    fn add_response_variable(&mut self, variable_name: &str, types: &str, value: Value) -> bool {
        ...
        for var_type in types {
            match var_type {
                ...
                _ => {
                    if conflict_types {
                        panic!(
                            "Conflicting types supplied for the variable {}",
                            variable_name
                        );
                    }
                    ...
                    default_value = match get_default_value(var_type) {
                        Ok(string) => string,
                        Err(_) => panic!("Unsupported type given for {}", variable_name),
                    };
        ...
        if !type_matched {
            self.variables
                .add(&variable_name, from_str(&default_value).unwrap());
            println!(
                "{} : Type not matching for the variable '{}'. Assigned the default value.",
                "WARN".yellow(),
                variable_name
            );
        } else {
            ...
            passed = match format.get(0..3).unwrap() {
                "\"{{" => match format.get(format.len() - 3..format.len()).unwrap() {
                    "}}\"" => {
                        ...
                        match format.find(":") {
                            Some(colon_position) => match format.find("&&") {
                                Some(first_and_position) => {
                                    let comparison = format
                                        .get((first_and_position + 2)..(format.len() - 3))
                                        .unwrap();
                                    let types = format
                                        .get((colon_position + 1)..first_and_position)
                                        .unwrap();

                                    let variable_name = format.get(3..colon_position).unwrap();

                                    let type_checked = self.add_response_variable(
                                        variable_name,
                                        types,
                                        response_value_org,
                                    );
                                    ...
                                    js_definitions
                                        .push_str(") {return true;} else {return false;}");
                                ...
                                None => {
                                    let variable_name = format.get(3..colon_position).unwrap();
                                    let types =
                                        format.get((colon_position + 1)..format.len() - 3).unwrap();
    ...
    pub fn parse_request_body(&mut self, body: Map<String, Value>) -> Map<String, Value> {
        fn parse_body(this: &mut Interpreter, body: Map<String, Value>) -> Map<String, Value> {
            ...
            for (k, v) in body {
                let new_val = match v {
                    Value::Object(obj) => Value::Object(parse_body(this, obj)),
                    Value::Array(arr) => Value::Array(
                        arr.iter()
                            .map(|val| from_str(&this.request_value(val)).unwrap())
                            .collect::<Vec<_>>(),
                    ),
                    _ => {
                                ...
                                while let Some(val) = arr_iter.next() {
                                    ...
                                    while let Some(val2) = arr2_iter.next() {
                                        match val2 {
                                            ...
                                            _ => {
                                                if !this
                                                    .response_value(val2.to_owned(), val.to_owned())
                                                {
                                                    arr_passed = false;
                                                }
                                            }
...
pub fn type_check(var_type: &str, value: &Value) -> bool {
    match value {
        Value::Number(_) => var_type == "number" || var_type == "any",
        Value::Array(_) => var_type == "array" || var_type == "any",
        Value::Bool(_) => var_type == "boolean" || var_type == "any",
        Value::Null => var_type == "null" || var_type == "any",
        Value::String(_) => var_type == "string" || var_type == "any",
        Value::Object(_) => var_type == "object" || var_type == "any",
    }
}
...
pub fn get_default_value(var_type: &str) -> Result<String, &str> {
    match var_type {
        "boolean" => Ok(String::from("false")),
        "number" => Ok(String::from("0")),
        "string" => Ok(String::from("\"\"")),
        "object" => Ok(String::from("{}")),
        "array" => Ok(String::from("[]")),
        "null" => Ok(String::from("null")),
        "any" => Ok(String::from("null")),
        _ => Err("Unssupported Type."),
    }
}
...
mod tests {
    ...
    pub fn test_response_value_with_normal_values() {
        ...
        assert_eq!(
            response_value(&mut interpreter, "1232443323", "1232443323"),
            true
        );

        assert_ne!(
            response_value(&mut interpreter, "1232443323", "123243323"),
            true
        );
    ...
    pub fn test_response_value_with_type_check() {
        ...
        assert_eq!(
            response_value(&mut interpreter, "\"{{string}}\"", "\"Foo\""),
            true
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{string| null}}\"", "null"),
            true
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{string| null}}\"", "\"Foo\""),
            true
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{string| null}}\"", "1232"),
            false
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{string}}\"", "1232"),
            false
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{boolean}}\"", "true"),
            true
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{array}}\"", "[1,2,3]"),
            true
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{object}}\"", "[1,2,3]"),
            false
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{object}}\"", "{\"a\":123}"),
            true
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{any}}\"", "[1,2,3]"),
            true
        );
    }
    ...
    pub fn test_response_value_with_variable_creation() {
        ...
        assert_eq!(
            response_value(&mut interpreter, "\"{{fooStr:string}}\"", "\"Foo\""),
            true
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{barStr:string|null}}\"", "null"),
            true
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{fooNmb:number}}\"", "1234"),
            true
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{fooNmbWrong:string}}\"", "1234"),
            false
        );

        assert_eq!(
            response_value(&mut interpreter, "\"{{fooArr:array}}\"", "[1,2,3]"),
            true
        );
    ...
    pub fn test_response_value_with_comparison() {
        ...
        assert_eq!(
            response_value(
                &mut interpreter,
                "\"{{barVal3:number&& barVal3<bar && barVal2< barVal3}}\"",
                "10"
            ),
            false
        );
        ...
        assert_eq!(
            response_value(
                &mut interpreter,
                "\"{{fooVal2:string&& (fooVal2!=foo|| fooVal2==fooVal1)}}\"",
                "\"Foo\""
            ),
            true
        );
        ...
        assert_eq!(
            response_value(
                &mut interpreter,
                "\"{{arrVal2:array&& arrVal2.length==arrVal.length}}\"",
                "[1,2,3,4]"
            ),
            false
        );
        ...
        assert_eq!(
            response_value(
                &mut interpreter,
                "\"{{objVal2:object&& objVal1.bar==objVal2.foo}}\"",
                "{\"foo\":5}"
            ),
            false
        );
    ...
    pub fn test_response_body_with_no_parameters() {
        ...
        let passed =
            interpreter.parse_response_body(response_body(r#"{}"#), response_body(r#"{}"#));
